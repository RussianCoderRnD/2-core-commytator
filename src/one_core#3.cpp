...
class MyDisplay                                                                // класс с именем Led
{                                                                              //
public:                                                                        // модификатор доступа
    void myDisplay(uint8_t x, uint8_t y, uint8_t size_font, uint8_t volts = 0) // функция включает светодиод
    {
    ...
    void myDisplay2(uint8_t x, uint8_t y, uint8_t size_font, String t = "") // функция включает светодиод
    {
        oled.setCursorXY(x, y); // курсор в (пиксель X, строка Y)
        oled.setScale(size_font);
        oled.print(t);
...
MyDisplay MyDis; // создание обьекта redLed класса Led
...
void loop()
{
    ...
    if (statusLineA == HIGH)
    {
        ...
        MyDis.myDisplay2(setX, setY + 1, 2, "Line A:");
        MyDis.myDisplay(setX + 84, setY + 1, 2, valA);
    }
    else
    {
    ...
    if (statusLineB == HIGH)
    {
        ...
        oled.invertText(true); // инвертируй текст!
        MyDis.myDisplay2(setX, setY + 24, 2, "Line B:");
        MyDis.myDisplay(setX + 84, setY + 24, 2, valB);
    }
    else
    {
    ...
    if (statusLineC == HIGH)
    {
        ...
        oled.invertText(true); // инвертируй текст!
        MyDis.myDisplay2(setX, setY + 48, 2, "Line C:");
        MyDis.myDisplay(setX + 84, setY + 48, 2, valC);
    }
    else
    {
