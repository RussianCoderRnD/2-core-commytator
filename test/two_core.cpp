...
void loop()
{
    ...
    Serial.print(String("valA ") + valA);
    Serial.print(String("  valB ") + valB);
    Serial.println(String(" valC ") + valC);

    if (valA >= lowVolts && valA <= hiVolts && valA > 150 && (flagLine_B + flagLine_C == false))
    ...
    if (valB >= lowVolts && valB <= hiVolts && valB > 150 && (flagLine_A + flagLine_C == false))
    {
        flagLine_B = 1;
    ...
    if (valC >= lowVolts && valC <= hiVolts && valC > 150 && (flagLine_A + flagLine_B == false))
    {
        flagLine_C = 1;
